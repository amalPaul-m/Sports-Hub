...
form.addEventListener('submit', async function (event) {
  ...
  if (!form.checkValidity() || croppedFiles.length < 0) {
    event.stopPropagation();
    imageError.style.display = croppedFiles.length < 0 ? 'block' : 'none';
...
let variantCount = parseInt(document.getElementById('counts').value) || 0;
